...
for index,row in sd_table.iterrows():
    ...
    mycursor.execute("INSERT IGNORE INTO rating (company_id, source_id, sd_rate, uncertainty, date, source_link) VALUES ((SELECT id from company WHERE name=%s), %s, %s, %s, NOW(), %s)", (row['company'], source_id, row['sd_rating'], 0, row['directory_link-href']))
    mydb.commit()
